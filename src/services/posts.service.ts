...
export const postsService = {
  ...
  /**
   * Get paginated list of posts by user ID
   * Uses Next.js caching with 60 second revalidation
   */
  async byUser(userId: number, limit = 20, cursor?: number): Promise<PaginatedResponse<Post>> {
    const params = new URLSearchParams({
      limit: limit.toString(),
    });
    ...
    return apiClient.get<PaginatedResponse<Post>>(
      `${API_ENDPOINTS.posts}/user/${userId}?${params.toString()}`,
      {
        revalidate: 60,
